  ...
  return (
    <ProfileLayout stats={stats ?? undefined}>
      <div className="space-y-4">
        <AnimeList animeInfoList={animeInfoList} route="dropped" />

        {pageCount > 1 && (
          <div className="flex justify-center items-center gap-4 pt-4">
            {pageNumber > 0 ? (
              <Button asChild variant="outline" size="sm">
                <Link href={`/my-anime/dropped?page=${pageNumber - 1}`}>Previous</Link>
              </Button>
            ) : (
            ...
            {pageNumber + 1 < pageCount ? (
              <Button asChild variant="outline" size="sm">
                <Link href={`/my-anime/dropped?page=${pageNumber + 1}`}>Next</Link>
              </Button>
            ) : (
